...
plot(blocs1[[1]]$DATE, blocs1[[1]]$Twater, type="l")
plot(blocs1[[2]]$DATE, blocs1[[2]]$Twater, type="l")
```
...
plot(aggregtout_P1$DATE, aggregtout_P1$Twater, type="p", pch=16, cex=0.4,main="Twater pour palier 1 avec données manquantes")
plot(P1EM$DATE, P1EM$X_imp[,1], type="p", pch=16, cex=0.4,main = "Twater pour palier 1 après imputation EM")
```
...
La volatilité instantanée décroît globalement avec le palier pour `Twater` (série plus lisse aux paliers élevés). Pour `O2sat` et `Chltot`, la volatilité typique varie peu selon le palier, mais on observe dans tous les cas des épisodes ponctuels de variations extrêmes (surement du à des outliers ou a des phénomènes très particulier qu'on ne pourrait expliquer).
